<template>
  <transition name="fade-section" key="fade__modal">
    <div class="modal__container" key="fade__modal" @click.self="$emit('closeModal', $event); clearInputs()">
        <Mod :msg="msg" :msgType="msgType" v-if="msg"/>
      <div class="modal">
        <div class="spans" @click="$emit('closeModal', $event); clearInputs()">
          <span></span>
          <span></span>
        </div>
        ...
        <div class="content" key="tab__flow">
          <transition name="flow-left" key="tab__flow__left">
            <div class="sign-in__content" key="tab__flow__sign-in" v-show="signIn">
              <div class="inputs">
                <div class="input" v-for="input in inputs.sign_in_inputs" :key="input.id">
                  <p class="error" v-show="input.error"> {{ input.inputMsg }} </p>
                  <input type="text" @input="passwordDots(input.id, $event)" class="bord" :id="input.id" :placeholder="input.placeholder"
                    @blur="checkInput($event)">
                  <img src="../assets/eye.svg" @click="showPass" class="passwordEye" alt="" v-if="input.img">
                </div>
              </div>
              <div class="buttons">
          ...
          <transition name="flow-right" key="tab__flow__right">
            <div class="sign-up__content" key="tab__flow__sign-up " v-show="!signIn">
              <div class="inputs">
...
<script>
  ...
  export default {
    ...
    methods: {
      showPass(e) {
          if (e.target.previousElementSibling.value == this.inputObj[e.target.previousElementSibling.id]) {
              e.target.previousElementSibling.value = this.inputObj[e.target.previousElementSibling.id].replace(/./gi, '*');
              e.target.style.opacity = '0.5';
          }
          else {
              e.target.previousElementSibling.value = this.inputObj[e.target.previousElementSibling.id];
      ...
      passwordDots(id, ev) {
        if (id === 'password') {
          if (ev.target.value.length < this.inputObj[id].length) {
            this.inputObj[id] = this.inputObj[id].slice(0, this.inputObj[id].length - 1);
            return;
          }
          if (ev.target.value.match(/./g)) this.inputObj[id] += ev.target.value.match(/[^*]/g);
      ...
      checkInput(event) {
        if (event.target.id === 'phone' && this.inputObj[event.target.id] != "") {
          ...
          if (/^[a-zA-z\d]+@[a-zA-Z\d]+\.\w{2,3}$/gi.test(this.inputObj[event.target.id])) {
            this.match[event.target.id] = true;
            event.target.classList.add('correct');
            event.target.classList.remove('wrong');
            this.inputs[`${this.currentTab}_inputs`][event.target.id].error = false;
            ...
            this.inputs[`${this.currentTab}_inputs`][event.target.id].error = true;
            this.inputs[`${this.currentTab}_inputs`][event.target.id].inputMsg =
              'Разрешены латинские буквы и цифры. "@" и .com обязательны!'
            this.match[event.target.id] = false;
            event.target.classList.add('wrong');
            event.target.classList.remove('correct');
          }
...
<style scoped>
 ...
 font-weight: 300;
  ...
  .flow-left-enter-active, .flow-left-leave-active, .flow-right-enter-active, .flow-right-leave-active {
    transition: all .5s;
  }
  .flow-left-enter {
